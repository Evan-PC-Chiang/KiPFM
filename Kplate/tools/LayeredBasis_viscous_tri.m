function U=LayeredBasis_viscous_tri(TriCenter,TriArea,dip,strike,slip,xloc,H,mu,lam,t,tR)
...
%shift coordinates to be centered on centroid
xy(:,1)=xy(:,1)-TriCenter(1);
xy(:,2)=xy(:,2)-TriCenter(2);

D=abs(TriCenter(3));  %depth of source
                      %depth is positive
...
%number of point sources is one
NL=1;
NW=1;
...
if ss~=0
	...
	G1(1:3:end)=TriArea*Gy_1;
	G1(2:3:end)=TriArea*Gx_1;
	G1(3:3:end)=-TriArea*Gz_1;
end

if ds~=0
   
	G2=zeros(3*length(Gy_2),1);
	G2(1:3:end)=TriArea*Gy_2;
	G2(2:3:end)=TriArea*Gx_2;
   G2(3:3:end)=-TriArea*Gz_2;
end

if ten~=0
   
	G3=zeros(3*length(Gy_3),1);
	G3(1:3:end)=TriArea*Gy_3;
	G3(2:3:end)=TriArea*Gx_3;
   G3(3:3:end)=-TriArea*Gz_3;
end
